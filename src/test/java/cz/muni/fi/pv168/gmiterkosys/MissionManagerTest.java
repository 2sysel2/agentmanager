...
public class MissionManagerTest {
	...
	public void testFindAllMissions() {
		...
		Mission mission1 = newMission(0, "testMission", "testistan", LocalDateTime.of(2000, 3, 1, 0, 0),
				LocalDateTime.of(2001, 1, 1, 1, 1), "failMission", Outcome.FAILED);
		Mission mission2 = newMission(1, "testMission", "testistan", LocalDateTime.of(2000, 3, 1, 0, 0),
				LocalDateTime.of(2001, 1, 1, 1, 1), "succesMission", Outcome.SUCCESSFUL);
	...
	public void testGetMissionByCode() {
		...
		try {
			missionManager.createMission(mission);
			assertThat("mission returned by getMissionByCode doesn't match mission created",
					missionManager.getMissionByCode(mission.getCode()), is(mission));
		} catch (ServiceFailureException ex) {
			Logger.getLogger(MissionManagerTest.class.getName()).log(Level.SEVERE, null, ex);
		}
	...
	public void testGetMissionById() {
		...
		try {
			missionManager.createMission(mission);
			assertThat("mission returned by getMissionById doesn't match mission created",
					missionManager.getMissionById(mission.getId()), is(mission));
		} catch (ServiceFailureException ex) {
			Logger.getLogger(MissionManagerTest.class.getName()).log(Level.SEVERE, null, ex);
		}
	...
	public void testUpdateMission() {
		...
		assertThat("updated mission doesn't equal mission parameter",
				missionManager.getMissionById(tempMission.getId()), is(tempMission));

	}

	@Test(expected = IllegalArgumentException.class)
	public void testStartAndEndDate() {
		Mission mission = newMission(0, "testMission", "testistan", LocalDateTime.of(2001, 1, 1, 1, 1),
				LocalDateTime.of(2000, 3, 1, 0, 0), "failMission", Outcome.FAILED);
		missionManager.createMission(mission);
	}
